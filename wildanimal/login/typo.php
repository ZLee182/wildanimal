...
<?php
	error_reporting(0);
	$link=mysql_connect("120.27.35.231","root","root") or die("失败".mysql_error());
...
<!-- typo -->
	...
	$link=mysql_connect("localhost","root","root") or die("失败".mysql_error());//连接服务器和数据库
...
<div class="distracted">
		  <h3 class="ghj">留言墙</h3>
					...
					error_reporting(0);//不显示任何报错。
					$link=mysql_connect("120.27.35.231","root","root") or die("失败".mysql_error());//连接服务器和数据库
					mysql_select_db("animaldesign") or die("数据库选择失败");//选择数据库
					mysql_query("set names utf8");//编码方式
